...
test('renders url and likes when button is pressed', async () => {
  ...
  window.localStorage.setItem('loggedBlogappUser', JSON.stringify(loggedUser[0]))
...
test('blogform returns the right details', async () => {
  const loggedUser = await userService.getAll()
  window.localStorage.setItem('loggedBlogappUser', JSON.stringify(loggedUser[0]))
  ...
  const mockHandler = jest.fn()
  const { container } = render(<BlogForm setCreateBlogVisible={mockHandler} sendNotificationMessageTimeout = {mockHandler}/>)
